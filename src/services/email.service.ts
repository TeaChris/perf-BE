...
class EmailService {
        ...
        async sendPasswordResetEmail(to: string, username: string, token: string) {
                ...
                try {
                        const { data, error } = await this.resend.emails.send({
                                from: `FlashRush <${ENVIRONMENT.EMAIL.FROM_EMAIL}>`,
                                to: [to],
                                subject: 'Reset your FlashRush password 🔐',
                                html: `
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body>
          <div class="container">
            ...
            <p>To reset your password, click the button below. This link is only valid for 1 hour.</p>
            <a href="${resetUrl}" class="button">Reset Password</a>
            <p style="margin-top: 32px; font-size: 14px;">If the button doesn't work, copy and paste this link into your browser:<br/>
            <span style="color: #f97316;">${resetUrl}</span></p>
            <div class="footer">
              &copy; 2026 FlashRush. All rights reserved. <br/>
              High-performance real-time engine for global shoppers.
            </div>
